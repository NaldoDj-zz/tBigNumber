...
#IFDEF __HARBOUR__
    ...
    Private __cSep AS CHARACTER VALUE Replicate("-",MaxCol())
...
#ENDIF
    ...
    #ELSE
        #IFDEF TBN_ARRAY
            __ConOut(fhLog,"USING       : " , ExeName() + " :: ARRAY")	//7
        #ELSE
            __ConOut(fhLog,"USING       : " , ExeName() + " :: STRING")	//7
        #ENDIF
    #ENDIF    

    #ifdef __POWMT__
        __ConOut(fhLog,"POWTHREAD   : " , "True")	//8
    #else
        __ConOut(fhLog,"POWTHREAD   : " , "False")	//8
    #endif

    #ifdef __ROOTMT__
        __ConOut(fhLog,"ROOTTHREAD  : " , "True")	//9
    #else
        __ConOut(fhLog,"ROOTTHREAD  : " , "False")	//9
    #endif

    #ifdef __SUBTMT__
        __ConOut(fhLog,"SUBTHREAD   : " , "True")	//10
    #else
        __ConOut(fhLog,"SUBTHREAD   : " , "False")	//10
    #endif

    #ifdef __MULTMT__
        __ConOut(fhLog,"MULTTHREAD  : " , "True")	//11
    #else
        __ConOut(fhLog,"MULTTHREAD  : " , "False")	//11
    #endif

	#IFDEF __HARBOUR__
		__ConOut(fhLog,"FINAL1      : " , "["+StrZero(__oRTime1:GetnTotal(),10)+"]|["+DtoC(__oRTime1:GetdEndTime())+"]["+__oRTime1:GetcEndTime()+"]|["+__oRTime1:GetcMediumTime()+"]") //12
		__ConOut(fhLog,"FINAL2      : " , "["+StrZero(__oRTime2:GetnTotal(),10)+"]|["+DtoC(__oRTime2:GetdEndTime())+"]["+__oRTime2:GetcEndTime()+"]|["+__oRTime2:GetcMediumTime()+"]") //13
	#ENDIF
	
    __ConOut(fhLog,__cSep)	//14
    __ConOut(fhLog,"")		//15
    __ConOut(fhLog,__cSep)	//16
	...
	#IFNDEF __PROTHEUS__
		...
		For w := 0 To 5
			...
			__oRTime2:SetRemaining(nISQRT/2)
...
#IFDEF __HARBOUR__
	...
	DispOutAt(12,0,"FINAL1      :  ["+StrZero(__oRTime1:GetnTotal(),10)+"]|["+DtoC(__oRTime1:GetdEndTime())+"]["+__oRTime1:GetcEndTime()+"]|["+__oRTime1:GetcMediumTime()+"]",'w+/n')
	DispOutAt(13,0,"FINAL2      :  ["+StrZero(__oRTime2:GetnTotal(),10)+"]|["+DtoC(__oRTime2:GetdEndTime())+"]["+__oRTime2:GetcEndTime()+"]|["+__oRTime2:GetcMediumTime()+"]",'w+/n')
    DEFAULT __nRow := 0
    IF ++__nRow >= __nMaxRow
...
#IFDEF __HARBOUR__
    Static Function cValToChar(e)
        ...
        ENDSWITCH
    Return(s)
    Static Procedure Progress(nDrow,nDcol)
        MEMVAR __oProgress
		DispOutAt( nDrow, nDcol, "[ ]" )
        DispOutAt( nDrow, nDcol+1, __oProgress:Eval() )
    Return
    Static Function BuildScreen(fhLog)
        CLEAR SCREEN
